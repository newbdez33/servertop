...
1. **顶栏**：品牌标识 + 服务器名 + Online 状态徽章 + OS/运行时间；右侧为刷新间隔徽章
   ...
   属客户端显示偏好，不算配置入口）
2. **指标卡片行**（4 张，信息密集型）：CPU（悬停显示型号）、Memory（含迷你堆叠条 +
   cache/swap）、Disk（分区告警变色 + 悬停显示全部分区）、Network（down/up 双线迷你图）
3. **图表区**：CPU 使用率面积图 + 网络 down/up 双线图（实时滚动，悬停查看数值）
4. **进程表 / Docker 容器**：Top N 进程（负载渐变色）；容器状态列表
5. **可选卡**（layout.json 启用）：memory/disk/system 详情卡、claude/codex 会话卡、llm 服务卡；
   主机 IP/内核/Agent 版本并入顶栏（meta + 悬停）
